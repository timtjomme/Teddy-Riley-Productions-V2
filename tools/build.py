...
def stats_html(n_releases, n_tracks):
    """Two big numbers under the homepage banner — every visible release
    and every track (each version counts on its own, same as the per-page
    counts main() prints) across every page in data/releases.json."""
...
def main():
    ...
    stats_changed = splice(ROOT / "index.html", STATS_START, STATS_END, stats_html(total_releases, total_tracks))
